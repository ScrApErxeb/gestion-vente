...
class VenteItem(db.Model):
    ...
    vente = db.relationship('Vente', backref=db.backref('items', lazy=True))
...
@login_required
def stats_produits_complete():
    """Statistiques des produits avec données réelles"""
    try:
        # Top produits vendus - CORRIGÉ
...
@login_required
def stats_clients_complete():
    """Statistiques des clients avec données réelles"""
    try:
        # Top clients par achats - CORRIGÉ
        top_clients = db.session.query(
            Client.id,
            Client.nom,
            Client.prenom,
...
def rapport_rentabilite():
    ...
    try:
        ...
        for vente in ventes:
            ...
            # car dans votre modèle, chaque vente a un seul produit
