...
async function saveEnterprise(event) {
    ...
    try {
        ...
        const response = await fetch('/api/system/parametres', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify([
                {cle: 'nom_entreprise', valeur: data.nom_entreprise, type_valeur: 'string'},
                {cle: 'adresse_entreprise', valeur: data.adresse, type_valeur: 'string'},
                {cle: 'telephone_entreprise', valeur: data.telephone, type_valeur: 'string'},
                {cle: 'email_entreprise', valeur: data.email, type_valeur: 'string'}
            ])
        });
...
async function saveGeneralParams(event) {
    event.preventDefault();
    const form = event.target;
    const submitBtn = form.querySelector('button[type="submit"]');
    const originalText = submitBtn.textContent;
    ...
    try {
        ...
        const data = {
            devise: document.getElementById('param-devise').value,
            tva: document.getElementById('param-tva').value,
            alertes_stock: document.getElementById('param-alertes').checked,
            emails_auto: document.getElementById('param-emails').checked
        };
        ...
        const response = await fetch('/api/system/parametres', {
            ...
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify([
                {cle: 'devise', valeur: data.devise, type_valeur: 'string'},
                {cle: 'tva_defaut', valeur: data.tva, type_valeur: 'number'},
                {cle: 'alertes_stock', valeur: data.alertes_stock ? '1' : '0', type_valeur: 'boolean'},
                {cle: 'emails_auto', valeur: data.emails_auto ? '1' : '0', type_valeur: 'boolean'}
            ])
        });
